"""HSI v1 weight-sensitivity at ±50% (v4.1 explicit spec — extends ±20% A6).

Same as hsi_sensitivity_analysis.py but at the v4.1-specified ±50%
perturbation magnitude. Tests whether HSI v1 AUC remains stable under
the larger weight excursions the original v4.1 design called for.

Output: data/hsi/v1/{site}_sensitivity_50pct.json
"""
...
def run_site(site, n_samples=64, perturb_pct=50):
    ...
    bands = da.values
    pyro = bands[6]; south = bands[5]; firerisk = bands[1]
    pine_tx = pyro * south
    ...
    for _ in range(n_samples):
        delta = rng.uniform(-p, p, size=4) * base
    ...
    if aucs:
        ...
        path = Path(f"data/hsi/v1/{site}_sensitivity_50pct.json")
        path.write_text(json.dumps(out, indent=2))
        print(f"  {site} sensitivity (+/-{perturb_pct}% perturb, n={n_samples}):")
        print(f"    AUC mean = {out['auc_mean']:.4f} +/- {out['auc_std']:.4f}")
        print(f"    AUC 5-95% range = [{out['auc_q05']:.4f}, {out['auc_q95']:.4f}]")
        print(f"    saved -> {path}")
        return out
...
def main():
    for site in ("uiseong", "sancheong"):
        print(f"\n=== {site} ===")
        run_site(site, n_samples=128, perturb_pct=50)
